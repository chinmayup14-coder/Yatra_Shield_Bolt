...
import { Shield, Mail, Lock, ArrowRight, ArrowLeft, UserCog } from 'lucide-react';
import { supabase } from '@/lib/supabase';

type GuideLoginPageProps = {
  onAuthSuccess: () => void;
  onBack: () => void;
};

export function GuideLoginPage({ onAuthSuccess, onBack }: GuideLoginPageProps) {
    ...
    <div className="min-h-screen bg-navy-900 flex items-center justify-center px-5 py-12 relative overflow-hidden">
      ...
      <div className="relative w-full max-w-md">
        ...
        <div className="bg-white rounded-3xl shadow-card p-8 sm:p-10">
          ...
          {/* Guide badge */}
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-navy-700/10 text-navy-600 text-xs font-bold tracking-wide uppercase mb-4">
            <UserCog className="w-3.5 h-3.5" />
            Guide Portal
          </div>

          <h1 className="text-2xl font-extrabold text-navy-700 mb-1">
            {mode === 'login' ? 'Guide Sign In' : 'Register as a Guide'}
          </h1>
          <p className="text-navy-400 text-sm mb-8">
            {mode === 'login'
              ? 'Sign in to manage your guide profile and connect with travellers.'
              : 'Create an account to join YATRA SHIELD as a verified local guide.'}
          </p>
          ...
          <div className="mt-6 text-center text-sm text-navy-400">
            {mode === 'login' ? "Don't have a guide account? " : 'Already have an account? '}
            <button
              onClick={() => {
